...
namespace Ichigo.Screens.Menus
{
    internal class MessageLogMenu : ScrollableTextMenu
    {
        ...
        private void PrintMessage(Message message, bool addNewline)
        {
            ...
            MessageBuffer.Surface.TimesShiftedUp = 0;
            ...
            for (int x = 0; x < MessageBuffer.Width; x++)
                MessageBuffer.Surface[x, 0].CopyAppearanceTo(_cachedTopRow[x], false);
            ...
            {
                if (MessageBuffer.Surface.TimesShiftedUp > 0)
                {
                    MessageBuffer.Surface.TimesShiftedUp--;
                    MessageBuffer.ShiftDown(1);
                    for (int x = 0; x < MessageBuffer.Width; x++)
                        _cachedTopRow[x].CopyAppearanceTo(MessageBuffer.Surface[x, 0], false);
                }
                else
                    Cursor.Position = Cursor.Position.Translate(0, -1);
            }

            _linesOfPreviousMessage = Cursor.Position.Y - oldY + 1 + MessageBuffer.Surface.TimesShiftedUp;
        }
    }
}
