...
namespace Ichigo.Engine.Screens.Surfaces
{
  public class MessageLogPanel : Console
  {
    ...
    private void MessageLogPanel_ParentChanged(object? sender, ValueChangedEventArgs<IScreenObject> e)
    {
      if (e.NewValue == null)
      {
        Core.Instance.MessageLog.MessageAdded -= MessageLog_MessageAdded;
        _firstMessage = true;
        Cursor.Position = (0, 0);
      }
      else
    ...
    {
      foreach (var message in Core.Instance.MessageLog.Messages)
    ...
    private void PrintMessage(Message message, bool addNewline)
    {
      ...
      for (int x = 0; x < Width; x++)
        Surface[x, 0].CopyAppearanceTo(_cachedTopRow[x], false);
      ...
      if (Surface.TimesShiftedUp > 0 && Cursor.Position.X == 0)
      {
        Surface.TimesShiftedUp--;

        Surface.ShiftDown(1);
        for (int x = 0; x < Width; x++)
          _cachedTopRow[x].CopyAppearanceTo(Surface[x, 0], false);
      }

      _linesOfPreviousMessage = Cursor.Position.Y - oldY + 1 + Surface.TimesShiftedUp;
    }
  }
}